...
window.PandaAddons.meteo = (function () {
  ...
  /* Packs d'icones : chaque pack est un dossier /static/wx/<pack>/ contenant
     les 15 memes noms de condition. La valeur est l'extension des fichiers.
     Le pack actif vient de la config (champ "pack" du manifeste). */
  const WXPACKS={meteocons:'svg',weathericon:'svg',colorline:'png',
                 meteoicons:'svg',weathericonic:'svg',makinthings:'svg'};
  let pack='meteocons';
  const wxUrl=n=>'/static/wx/'+pack+'/'+n+'.'+(WXPACKS[pack]||'svg');
  ...
  function ensureCss(){if(document.getElementById('wxUiCss'))return;const s=document.createElement('style');s.id='wxUiCss';s.textContent=CSS+"\n.wemoji .wxi{width:66px;height:66px;display:block}.wdce .wxi{width:44px;height:44px;display:inline-block}.whour .he .wxi{width:38px;height:38px;display:inline-block}.stub .big .wxi{width:72px;height:72px;display:inline-block}#wxBar .wxi{width:24px;height:24px;display:inline-block;vertical-align:-7px;margin-right:2px}"+"\n.wemoji,.wdce,.whour .he,.wsi,.wdcp,.wdcuv,.stub .big{font-family:'Noto Color Emoji','Segoe UI Emoji','Apple Color Emoji',sans-serif}";document.head.appendChild(s);}
  function wc(code,isDay){
    // Icones servies depuis /static/wx/<pack>/ : fichiers separes, donc
    // caches par le navigateur et sans collision possible entre les
    // identifiants de degrades des differentes icones.
    ...
    return ['<img class="wxi" src="'+wxUrl(n)+'" alt="" draggable="false">',e[2],n];
  }
  function getGeo(){
  ...
  async function refresh(sdk){
    ...
    if(cfg.pack&&WXPACKS[cfg.pack])pack=cfg.pack;
  ...
  function hourCurveSVG(hours,isDay){
    ...
    hours.forEach((x,i)=>{const hw=wc(x.code,isDay);
      g+='<circle cx="'+X(i).toFixed(1)+'" cy="'+Y(x.temp).toFixed(1)+'" r="3" fill="var(--accent)"/>'+
         ...
         '<image href="'+wxUrl(hw[2])+'" x="'+(X(i)-12).toFixed(1)+'" y="'+(H-50)+'" width="24" height="24"/>'+
